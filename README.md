...
3. Step into the gym folder
   `cd gym`
4. Run docker
   `docker-compose up`
5. Open a new terminal, run the following command, and copy `laravel.test` container id:
   `docker ps`
6. Step into the container and run the migration command:
    ```
    docker exec -it <container-id> bash
    php artisan migrate:fresh --seed
    ```
7. Run the app in the browser: `http://localhost:80`
